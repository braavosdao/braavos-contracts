...
describe("Bonding", () => {
  ...
  describe("adjust", () => {
    it("should able to adjust with bcv <= 40", async () => {
      ...
      const maxBondPayout = 1000; // 1000 = 1% of BRAVO total supply
  ...
  describe("deposit", () => {
    it("should get vested fully", async () => {
      ...
      const prevDAOReserve = await bravo.balanceOf(daoAddr);
      ...
      await daiBond.deposit(depositAmount, largeApproval, deployer.address);
      console.log(
        "dao balance: " + formatUnits(await bravo.balanceOf(daoAddr), 9)
      );
      expect(await bravo.balanceOf(daoAddr)).to.eq("35834236186");

      await timeAndMine.setTimeIncrease(20);
      await expect(() =>
        daiBond.redeem(deployer.address, false)
      ).to.changeTokenBalance(bravo, deployer, "30834236186");
    });
    ...
    it("should get vested partially", async () => {
      ...
      const totalBravo = depositAmount
        .div(bondPrice)
      ...
      await expect(() =>
        daiBond.redeem(deployer.address, false)
      ).to.changeTokenBalance(bravo, deployer, totalBravo.div(5));

      // fully vested, get rest 80%
      await timeAndMine.setTimeIncrease(10);
      await expect(() =>
        daiBond.redeem(deployer.address, false)
      ).to.changeTokenBalance(bravo, deployer, totalBravo - totalBravo.div(5));
    });
    ...
    it("should staked directly", async () => {
      ...
      expect(await sBravo.balanceOf(deployer.address)).to.eq("5000000000");
    });
  });
});
